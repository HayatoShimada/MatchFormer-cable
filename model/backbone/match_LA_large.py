...
# Original file: model/backbone/match_LA_large.py (upstream commit 1b2da5c).
#
# Mask-aware linear-attention surgery: zero K and V at masked tokens before
# the kernel sum so they cannot accumulate into KV; the Z denominator also
# excludes them. Backwards compatible: mask=None reproduces upstream exactly.
import torch
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if self.cross == True:
            ...
            if mask is not None:
                # mask is [B, N]; under cross-attention the K/V come from the
                # *other* image, so swap halves to align with key/value.
        ...
        if k_mask is not None:
            # Zero K and V at masked positions so they contribute 0 to KV/Z.
            m = k_mask[..., None, None].float()  # [B, N, 1, 1]
            K = K * m
            value = value * m
