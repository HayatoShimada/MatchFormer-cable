...
# Original file: model/backbone/match_SEA_lite.py (upstream commit 1b2da5c).
# Same mask-aware-attention surgery as match_SEA_large.py; see that file's
...
class Attention(nn.Module):
    ...
    def _kv_mask(self, mask, H, W):
        ...
        return _downsample_mask(m2d, h2, w2).reshape(B, -1)

    def forward(self, x, H, W, mask=None):
        B, N, C = x.shape
        kv_mask = self._kv_mask(mask, H, W)
        if self.cross == True:
            MiniB = B // 2
